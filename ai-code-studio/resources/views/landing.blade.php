...
@php
  ...
  $features = array_map(fn ($f) => [$f[0], __($f[1]), __($f[2])], $features);
  $how = array_map(fn ($h) => [$h[0], __($h[1]), __($h[2])], $how);
  $faqs = array_map(fn ($f) => [__($f[0]), __($f[1])], $faqs);
  $buildables = array_map(fn ($b) => __($b), $buildables);
  $cardPlans = $plans->map(fn ($p) => [
    'name' => $p->name, 'desc' => $p->description, 'items' => $p->features ?? [], 'tag' => $p->tag,
    'm' => $p->isCustom() ? null : $p->price_cents / 100, 'cta' => __($p->cta ?: 'Get started'), 'hi' => $p->highlighted,
    'fm' => $p->isCustom() ? '' : \App\Services\Billing\Billing::format(\App\Services\Billing\Billing::amountFor($p, 'monthly')),
    'fy' => $p->isCustom() ? '' : \App\Services\Billing\Billing::format((int) round(\App\Services\Billing\Billing::amountFor($p, 'yearly') / 12)),
  ]);
@endphp
...
<header style="position:sticky;top:0;z-index:10;background:rgba(10,10,12,0.85);backdrop-filter:blur(10px);border-bottom:1px solid #16161B">
  <div style="max-width:1200px;margin:0 auto;padding:0 24px;height:60px;display:flex;align-items:center;gap:28px">
    ...
    <nav data-r="hidem" style="display:flex;gap:22px;font-size:14px;flex:1;flex-wrap:wrap">
      <a href="#features" style="color:#A5A5AF">{{ __('Product') }}</a><a href="#build" style="color:#A5A5AF">{{ __('What you can build') }}</a>@if ($showPricing)<a href="#pricing" style="color:#A5A5AF">{{ __('Pricing') }}</a>@endif<a href="#faq" style="color:#A5A5AF">FAQ</a>
    </nav>
    <div style="display:flex;gap:8px;align-items:center;margin-left:auto">
      @auth
        <a href="{{ route('studio.dashboard') }}" style="height:36px;padding:0 16px;border-radius:7px;background:var(--accent);color:#fff;font-size:14px;font-weight:500;display:flex;align-items:center">{{ __('Open studio') }}</a>
      @else
        <a href="{{ route('login') }}" style="font-size:14px;color:#D4D4DA;padding:0 10px">{{ __('Sign in') }}</a>
        <a href="{{ route('register') }}" class="btn primary" style="height:36px;padding:0 16px;font-size:14px">{{ __('Start building') }}</a>
      @endauth
    </div>
  </div>
</header>
...
<section style="max-width:1200px;margin:0 auto;padding:88px 24px 40px;display:flex;flex-direction:column;align-items:center;text-align:center">
  <div style="display:flex;align-items:center;gap:8px;padding:5px 12px;border:1px solid #26222F;border-radius:20px;font-size:12.5px;color:#B4A5FF;margin-bottom:26px;font-family:'Geist Mono',ui-monospace,monospace">{{ __('IDEATE → PLAN → CODE → TEST → PREVIEW → DEPLOY') }}</div>
  ...
  <p style="margin:22px 0 0;font-size:18px;line-height:1.55;color:#9A9AA5;max-width:620px;text-wrap:pretty">{{ __('Describe your idea. A team of AI agents plans it, writes the code, checks it and publishes it — while you keep full control of every file and change.') }}</p>
  <form method="GET" action="{{ auth()->check() ? route('studio.new') : route('register') }}" style="margin-top:34px;width:min(680px,100%);border:1px solid #2A2540;background:#110F1A;border-radius:12px;padding:14px;display:flex;flex-direction:column;gap:12px;text-align:left">
    <input name="idea" x-model="idea" placeholder="{{ __('Describe what you want to build…') }}" style="background:transparent;border:none;outline:none;color:#F2F2F5;font-size:16px;font-family:inherit;padding:4px 4px">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap">
      ...
      <button class="btn primary" style="height:36px;padding:0 16px;font-size:14px">{{ __('Generate plan →') }}</button>
    </div>
  </form>
  <div style="margin-top:14px;font-size:13px;color:#6E6E79">{{ __('Free to start · No credit card · Export your code anytime') }}</div>
</section>
  ...
  <div style="border:1px solid #1F1F26;border-radius:14px;overflow:hidden;background:#0D0D10;box-shadow:0 40px 100px rgba(0,0,0,0.5)">
    ...
    <div data-r="mockgrid" style="display:grid;grid-template-columns:minmax(0,180px) minmax(0,1fr) minmax(0,300px);min-height:380px">
      ...
      <div data-r="mockside" style="border-left:1px solid #1C1C22;padding:14px;display:flex;flex-direction:column;gap:10px;text-align:left">
        ...
        <div style="margin-top:auto;display:flex;gap:6px"><span style="flex:1;height:28px;border-radius:6px;background:var(--accent);color:#fff;font-size:12px;display:grid;place-items:center">{{ __('Approve') }}</span><span style="height:28px;padding:0 10px;border-radius:6px;border:1px solid #2A2A32;font-size:12px;display:grid;place-items:center;color:#C9C9D1">{{ __('Review') }}</span></div>
      </div>
    </div>
  </div>
</section>
...
<section id="features" style="max-width:1200px;margin:0 auto;padding:60px 24px">
  <div style="max-width:620px;margin-bottom:40px">
    <div style="font-family:'Geist Mono',ui-monospace,monospace;font-size:12px;color:#A99BFF;margin-bottom:12px">{{ __('PRODUCT') }}</div>
    <h2 style="margin:0;font-size:clamp(30px,4vw,44px);letter-spacing:-0.03em;font-weight:600;line-height:1.1">{{ __('An AI coding team, not a chatbot.') }}</h2>
  </div>
...
<section style="max-width:1200px;margin:0 auto;padding:60px 24px">
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:40px;align-items:start">
    <div>
      <div style="font-family:'Geist Mono',ui-monospace,monospace;font-size:12px;color:#A99BFF;margin-bottom:12px">{{ __('HOW IT WORKS') }}</div>
      <h2 style="margin:0;font-size:clamp(30px,4vw,44px);letter-spacing:-0.03em;font-weight:600;line-height:1.1">{{ __('From one sentence to a deployed product.') }}</h2>
    </div>
...
<section id="build" style="max-width:1200px;margin:0 auto;padding:60px 24px">
  <div style="font-family:'Geist Mono',ui-monospace,monospace;font-size:12px;color:#A99BFF;margin-bottom:12px">{{ __('WHAT YOU CAN BUILD') }}</div>
  <h2 style="margin:0 0 28px;font-size:clamp(30px,4vw,44px);letter-spacing:-0.03em;font-weight:600;line-height:1.1;max-width:700px">{{ __('Any language. Any framework. Any kind of software.') }}</h2>
...
<section id="pricing" style="max-width:1200px;margin:0 auto;padding:60px 24px">
  <div style="display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;gap:16px;margin-bottom:28px">
    <div><div style="font-family:'Geist Mono',ui-monospace,monospace;font-size:12px;color:#A99BFF;margin-bottom:12px">{{ __('PRICING') }}</div><h2 style="margin:0;font-size:clamp(30px,4vw,44px);letter-spacing:-0.03em;font-weight:600">{{ __('Start free. Scale when you ship.') }}</h2></div>
    <div style="display:flex;border:1px solid #1F1F26;border-radius:8px;padding:3px;gap:2px">
      <span @click="yearly = false" :style="{ background: yearly ? 'transparent' : 'var(--accent-soft)', color: yearly ? '#8A8A94' : '#E4E4E9' }" style="padding:6px 14px;border-radius:6px;font-size:13.5px;cursor:pointer">{{ __('Monthly') }}</span>
      <span @click="yearly = true" :style="{ background: yearly ? 'var(--accent-soft)' : 'transparent', color: yearly ? '#E4E4E9' : '#8A8A94' }" style="padding:6px 14px;border-radius:6px;font-size:13.5px;cursor:pointer">{{ __('Yearly · save 20%') }}</span>
    </div>
  </div>
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:14px">
    @foreach ($cardPlans as $p)
      <div style="border:1px solid {{ $p['hi'] ? 'var(--accent-line)' : '#1C1C22' }};background:{{ $p['hi'] ? '#110F1A' : '#0D0D10' }};border-radius:12px;padding:22px;display:flex;flex-direction:column;gap:16px">
        ...
        <div>
          @if ($p['m'] === null)
            <span style="font-size:36px;font-weight:600;letter-spacing:-0.03em">{{ __('Custom') }}</span>
          @else
            <span style="font-size:36px;font-weight:600;letter-spacing:-0.03em" x-text="yearly ? @js($p['fy']) : @js($p['fm'])">{{ $p['fm'] }}</span>@if ($p['m'] > 0)<span style="color:#6E6E79;font-size:14px"> {{ __('/ month') }}</span>@endif
          @endif
        </div>
...
<section id="faq" style="max-width:820px;margin:0 auto;padding:60px 24px">
  <h2 style="margin:0 0 24px;font-size:clamp(28px,3.6vw,38px);letter-spacing:-0.03em;font-weight:600">{{ __('Questions') }}</h2>
...
<section style="max-width:1200px;margin:0 auto;padding:60px 24px 90px">
  <div style="border:1px solid #2A2540;background:#110F1A;border-radius:16px;padding:56px 32px;text-align:center;display:flex;flex-direction:column;align-items:center;gap:18px">
    <h2 style="margin:0;font-size:clamp(30px,4.4vw,48px);letter-spacing:-0.03em;font-weight:600;line-height:1.08;text-wrap:balance">{{ __('One idea → complete software.') }}</h2>
    <p style="margin:0;color:#9A9AA5;font-size:17px">{{ __('Your first project is free.') }}</p>
    <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center">
      <a href="{{ route('register') }}" class="btn primary lg">{{ __('Start building free') }}</a>
      <a href="{{ auth()->check() ? route('studio.dashboard') : route('login') }}" style="height:42px;padding:0 20px;border-radius:8px;border:1px solid #2A2A32;color:#E4E4E9;display:flex;align-items:center">{{ __('See the studio') }}</a>
    </div>
  </div>
</section>
...
<footer style="border-top:1px solid #16161B">
  <div style="max-width:1200px;margin:0 auto;padding:40px 24px;display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:28px;font-size:14px">
    ...
    <div style="display:flex;flex-direction:column;gap:9px"><span style="color:#6E6E79;font-size:12px;font-family:'Geist Mono',ui-monospace,monospace">{{ __('PRODUCT') }}</span><a href="#features" style="color:#A5A5AF">{{ __('Features') }}</a>@if ($showPricing)<a href="#pricing" style="color:#A5A5AF">{{ __('Pricing') }}</a>@endif<a href="#build" style="color:#A5A5AF">{{ __('What you can build') }}</a></div>
    <div style="display:flex;flex-direction:column;gap:9px"><span style="color:#6E6E79;font-size:12px;font-family:'Geist Mono',ui-monospace,monospace">{{ __('ACCOUNT') }}</span><a href="{{ route('login') }}" style="color:#A5A5AF">{{ __('Sign in') }}</a><a href="{{ route('register') }}" style="color:#A5A5AF">{{ __('Create account') }}</a></div>
    <div style="display:flex;flex-direction:column;gap:9px"><span style="color:#6E6E79;font-size:12px;font-family:'Geist Mono',ui-monospace,monospace">{{ __('COMPANY') }}</span>@if ($support = \App\Support\Settings::get('support_email'))<a href="mailto:{{ $support }}" style="color:#A5A5AF">{{ __('Contact support') }}</a>@endif<a href="{{ route('terms') }}" style="color:#A5A5AF">{{ __('Terms') }}</a><a href="{{ route('privacy') }}" style="color:#A5A5AF">{{ __('Privacy') }}</a><span>@include('partials.language-switcher')</span></div>
  </div>
</footer>
</div>
@endsection
