...
<div data-screen-label="Plans" style="display:flex;flex-direction:column;gap:20px">
  ...
  <div style="border:1px solid #1C1C22;border-radius:11px;background:#0E0E11">
    <div style="padding:14px 18px;border-bottom:1px solid #1C1C22"><div style="font-weight:600">Payment gateways</div><div style="font-size:12.5px;color:#8A8A94;margin-top:3px">Where your customers’ payments go. Save your keys here; checkout uses the first connected gateway.</div></div>
    @foreach ($G as $key => [$ini, $name, $d, $docs])
      ...
      <div x-data="{ open: false }" style="border-bottom:1px solid #16161B">
        ...
        <form class="d-grid" x-show="open" x-cloak method="POST" action="{{ route('admin.gateways.update', $key) }}" style="padding:0 18px 14px 66px;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:10px;align-items:end">@csrf
          <label class="lbl"><span>Public / client key</span><input class="inp sm mono" name="public" value="{{ $gateways[$key]['public'] ?? '' }}"></label>
          <label class="lbl"><span>Secret key</span><input class="inp sm mono" type="password" name="secret" placeholder="{{ $on ? '•••••••• (saved)' : '' }}" required></label>
          <div style="display:flex;gap:6px"><button class="btn primary sm" style="height:34px">Save</button>@if ($on)<button name="disconnect" value="1" class="btn danger sm" style="height:34px" formnovalidate>Disconnect</button>@endif<a href="{{ $docs }}" target="_blank" rel="noopener" class="btn ghost sm" style="height:34px">Get keys ↗</a></div>
        </form>
      </div>
    @endforeach
  </div>
