...
<form method="POST" action="{{ route('onboarding') }}" x-data="{ level: {{ $level }} }" style="display:flex;flex-direction:column;gap:18px">
  ...
  <div style="font-family:'Geist Mono',ui-monospace,monospace;font-size:11.5px;color:#6E6E79">{{ __('ONE QUICK QUESTION') }}</div>
  <div><h1 style="margin:0;font-size:26px;font-weight:600;letter-spacing:-0.02em">{{ __('How much coding do you do?') }}</h1><p style="margin:6px 0 0;color:#8A8A94">{{ __('We’ll set up the workspace to match. Change it any time.') }}</p></div>
  ...
  <button class="btn primary lg">{{ __('Continue') }}</button>
</form>
@endsection
