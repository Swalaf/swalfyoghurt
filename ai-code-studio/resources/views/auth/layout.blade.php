...
<div style="min-height:100vh;display:flex;background:#0A0A0C;font-size:14px">
  <div style="flex:1;min-width:0;display:flex;flex-direction:column;padding:28px 32px">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap">
      ...
      @auth
        <form method="POST" action="{{ route('logout') }}" class="inline"><button style="background:none;border:none;color:#6E6E79;font-size:12.5px;cursor:pointer">Sign out</button></form>
      @endauth
    </div>
    ...
    <div style="font-size:12px;color:#55555F;display:flex;gap:16px;flex-wrap:wrap"><span>© {{ date('Y') }} {{ \App\Support\Settings::get('company_name') }}</span>@if ($s = \App\Support\Settings::get('support_email'))<a href="mailto:{{ $s }}" style="color:#6E6E79">Support</a>@endif</div>
  </div>
  ...
  <div data-r="side" style="width:46%;max-width:720px;flex:none;border-left:1px solid #1C1C22;background:#0D0D10;display:flex;flex-direction:column;justify-content:center;padding:48px;gap:28px">
    <div style="font-family:'Geist Mono',ui-monospace,monospace;font-size:12px;color:#A99BFF">IDEATE → PLAN → CODE → TEST → PREVIEW → DEPLOY</div>
    <h2 style="margin:0;font-size:clamp(28px,3vw,40px);letter-spacing:-0.03em;font-weight:600;line-height:1.1;text-wrap:balance">Describe it. Approve the plan. Ship it.</h2>
