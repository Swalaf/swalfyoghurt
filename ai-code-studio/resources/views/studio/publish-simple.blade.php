...
<div data-screen-label="Publish (Simple)" style="flex:1;overflow:auto;padding:36px 28px 60px" @if ($running) x-data x-init="setTimeout(() => location.reload(), 1500)" @endif>
  <div style="max-width:760px;margin:0 auto;display:flex;flex-direction:column;gap:20px">
    <div><h1 style="margin:0;font-size:26px;font-weight:600;letter-spacing:-0.02em">{{ __('Publish your app') }}</h1><p style="margin:6px 0 0;color:#8A8A94">Put {{ $project->name }} on the internet so anyone can use it.</p></div>
      ...
      @else
        @if ($current && $current->status === 'live')
          <div style="display:flex;flex-direction:column;gap:14px;padding-bottom:16px;border-bottom:1px solid #1C1C22">
            <div style="display:flex;gap:12px;align-items:center"><span style="width:40px;height:40px;border-radius:50%;background:#15261C;color:#58C98A;display:grid;place-items:center;font-size:18px">✓</span><div><div style="font-weight:600;font-size:16px">{{ __('Your app is live!') }}</div><div style="color:#8A8A94;font-size:13px">{{ __('Anyone with the link can use it now.') }}</div></div></div>
            @php($url = \App\Jobs\DeployProject::url($current->subdomain))
            <div x-data="{ copied: false }" style="display:flex;align-items:center;gap:8px;border:1px solid #26262E;border-radius:9px;background:#111115;padding:0 6px 0 14px;height:44px"><span class="mono" style="flex:1;font-size:13.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ $url }}</span><span @click="navigator.clipboard.writeText(@js($url)); copied = true" class="btn sm" style="height:32px" x-text="copied ? 'Copied' : 'Copy'">{{ __('Copy') }}</span><a href="{{ $url }}" target="_blank" class="btn primary sm" style="height:32px">{{ __('Open') }}</a></div>
          </div>
        @elseif ($current && $current->status === 'failed')
          <div style="padding:10px 12px;border-radius:8px;background:#1A0F0F;color:#F2B8B2;font-size:13px;line-height:1.5">That didn’t work: {{ collect($current->log)->last()['m'] ?? 'unknown error' }}. <a href="{{ route('studio.projects.builder', $project) }}">{{ __('Ask the AI to fix it →') }}</a></div>
        @endif
        <form method="POST" action="{{ route('studio.projects.deploy.store', $project) }}" style="display:flex;flex-direction:column;gap:14px">
          @csrf
          <div style="font-weight:600;font-size:15px">{{ __('Your web address') }}</div>
          ...
          <div style="font-size:13px;color:#8A8A94">{{ __('Want your own domain like') }} <span style="color:#C9C9D1">{{ __('mybusiness.com') }}</span>{{ __('? Download your app from the Code view and host it anywhere — it’s just files.') }}</div>
    ...
    <div style="border:1px solid #1C1C22;border-radius:14px;background:#0E0E11">
      <div style="padding:14px 18px;border-bottom:1px solid #1C1C22;font-weight:600">{{ __('Earlier versions') }}</div>
      @forelse ($deployments->where('status', '!=', 'running') as $h)
        <div style="display:flex;align-items:center;gap:12px;padding:12px 18px;border-bottom:1px solid #16161B"><span style="width:8px;height:8px;border-radius:50%;background:{{ $h->status === 'failed' ? '#E5695E' : '#58C98A' }};flex:none"></span><div style="flex:1;min-width:0"><div>{{ $h->message }}</div><div style="font-size:12px;color:#6E6E79">{{ $h->created_at->diffForHumans() }}{{ $h->status === 'live' ? ' · live now' : '' }}{{ $h->status === 'failed' ? ' · didn’t publish' : '' }}</div></div>
          @if ($h->status === 'superseded' && $h->hasFiles())<form method="POST" action="{{ route('studio.projects.deploy.rollback', [$project, $h]) }}" class="inline">@csrf<button style="font-size:12.5px;color:#C7BDFF;background:none;border:none;cursor:pointer">{{ __('Go back to this') }}</button></form>@endif
        </div>
      @empty
        <div style="padding:14px 18px;color:#6E6E79;font-size:13px">{{ __('Nothing published yet.') }}</div>
      @endforelse
    </div>
  </div>
</div>
