...
<div data-screen-label="Builder" x-data="builder(@js($state), @js([
    ...
    <div x-ref="feed" style="flex:1;overflow-y:auto;overflow-x:hidden;padding:16px;display:flex;flex-direction:column;gap:14px;min-width:0">
      ...
      <div style="display:flex;gap:10px">
        ...
        <div style="flex:1;display:flex;flex-direction:column;gap:10px;line-height:1.5;min-width:0">
          <div>I planned <b style="font-weight:600">{{ count($project->spec['features'] ?? []) }} features</b> across {{ count($project->spec['pages'] ?? []) }} pages. <span x-text="s.tasks.length ? 'Here’s where I am:' : 'Start the build whenever you’re ready.'"></span></div>
          ...
          <form x-show="!s.tasks.length || s.status === 'failed'" method="POST" action="{{ route('studio.projects.build', $project) }}">@csrf<button class="btn primary" x-text="s.status === 'failed' ? 'Try the build again' : 'Start building'">Start building</button></form>
        </div>
      </div>
      <template x-for="m in s.messages" :key="m.id">
        <div class="bub" :class="m.role === 'user' ? 'user' : 'ai'" x-text="m.text"></div>
      </template>
      <div class="d-flex" x-show="thinking" x-cloak style="align-self:flex-start;gap:8px;align-items:center;color:#8A8A94;font-size:12.5px"><span style="color:#A99BFF">✦</span>Working on it…</div>
      <div class="d-flex" x-show="s.pending" x-cloak style="border:1px solid #3A3160;background:#110F1A;border-radius:12px;padding:14px;flex-direction:column;gap:10px">
        <div style="font-size:11.5px;color:#C7BDFF;font-weight:500">NEEDS YOUR OK</div>
        <div style="line-height:1.5" x-text="s.pending?.summary"></div>
        <div style="font-size:12px;color:#8A8A94"><span x-text="s.pending?.files.length"></span> file(s) · <span style="color:#58C98A" x-text="'+' + (s.pending?.additions || 0)"></span> <span style="color:#E5695E" x-text="'−' + (s.pending?.deletions || 0)"></span> lines · safe to undo later</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap"><button @click="decide('approve')" class="btn primary" style="height:34px">Yes, go ahead</button><button @click="send('Explain the changes you just proposed in plain language.', 'Ask')" class="btn ghost" style="height:34px">Explain more</button><button @click="decide('reject')" class="btn danger" style="height:34px;border:none">Discard</button></div>
      </div>
    </div>
  ...
  <div data-r="bprev" style="flex:1;min-width:0;display:flex;flex-direction:column;background:#111114">
    <div style="min-height:48px;flex:none;display:flex;align-items:center;gap:8px;padding:8px 14px;border-bottom:1px solid #1C1C22;background:#0D0D10;flex-wrap:wrap">
      ...
      <span @click="undo()" :style="{ opacity: s.canUndo ? 1 : .45, cursor: s.canUndo ? 'pointer' : 'default' }" style="height:30px;padding:0 11px;border-radius:7px;border:1px solid #1F1F26;display:flex;align-items:center;font-size:12.5px;color:#C9C9D1;flex:none">↶ Undo</span>
      ...
      <a href="{{ route('studio.projects.deploy', $project) }}" class="btn primary" style="height:30px;font-size:12.5px;flex:none">Publish</a>
    </div>
    <div style="flex:1;overflow:auto;padding:20px;display:flex;justify-content:center;position:relative">
      <iframe x-ref="frame" :src="frameSrc" sandbox="allow-scripts allow-forms allow-popups allow-modals" title="App preview" :style="{ width: device === 'phone' ? '390px' : device === 'tablet' ? '768px' : '100%' }" style="max-width:100%;min-height:100%;height:100%;border:0;background:#FAFAF8;border-radius:10px;box-shadow:0 20px 60px rgba(0,0,0,0.4)"></iframe>
      <div class="d-flex" x-show="sel" x-cloak style="position:absolute;right:28px;top:28px;z-index:5;width:min(320px,calc(100% - 56px));background:#0E0E11;color:#E4E4E9;border:1px solid #3A3160;border-radius:10px;padding:12px;flex-direction:column;gap:10px;box-shadow:0 16px 40px rgba(0,0,0,0.4)">
        ...
        <div style="display:flex;gap:6px"><input x-model="editText" @keydown.enter="applyEdit(editText)" placeholder="Or describe the change…" class="inp sm" style="flex:1;min-width:0;height:30px;font-size:12.5px"><span @click="applyEdit(editText)" style="height:30px;padding:0 10px;border-radius:6px;background:var(--accent);color:#fff;display:flex;align-items:center;font-size:12px;cursor:pointer">Apply</span></div>
      </div>
    </div>
  </div>
</div>
