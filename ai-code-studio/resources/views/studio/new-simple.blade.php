...
<div data-screen-label="Create (Simple)" style="flex:1;overflow:auto;padding:36px 28px 60px">
  <div style="max-width:880px;margin:0 auto;display:flex;flex-direction:column;gap:26px">
    ...
    <form method="POST" action="{{ route('studio.projects.store') }}" x-data="{ kind: @js(old('kind', 'Web app')), busy: false }" @submit="busy = true" style="display:flex;flex-direction:column;gap:18px">
      ...
      <div><h1 style="margin:0;font-size:26px;font-weight:600;letter-spacing:-0.02em">{{ __('What kind of thing are you making?') }}</h1><p style="margin:6px 0 0;color:#8A8A94">{{ __('Pick the closest one — you can change anything later.') }}</p></div>
      ...
      <label class="lbl"><span>{{ __('Describe it in your own words') }}</span>
        <textarea name="idea" required minlength="5" class="inp" placeholder="{{ __('e.g. A website for my bakery where people can see the menu and order cakes for pickup') }}" style="height:auto;min-height:90px;padding:10px 12px;line-height:1.5;resize:vertical">{{ old('idea', $idea) }}</textarea>
        @error('idea')<span class="err">{{ $message }}</span>@enderror
      </label>
      <div style="display:flex;justify-content:flex-end"><button class="btn primary" :style="busy ? { opacity: .6, pointerEvents: 'none' } : {}" style="height:40px;padding:0 20px;border-radius:9px;font-size:14px" x-text="busy ? 'Planning your app…' : 'Next →'">{{ __('Next →') }}</button></div>
    </form>
    @else
    ...
    <form method="POST" action="{{ route('studio.projects.plan.update', $project) }}" x-data="{ off: {}, busy: false }" @submit="busy = true" style="display:flex;flex-direction:column;gap:18px">
      ...
      <div><h1 style="margin:0;font-size:26px;font-weight:600;letter-spacing:-0.02em">{{ __('Here’s what I’ll build for you') }}</h1><p style="margin:6px 0 0;color:#8A8A94">{{ __('Read it through. Untick anything you don’t want, or tell me what to add.') }}</p></div>
      <div style="border:1px solid #1C1C22;border-radius:12px;background:#0E0E11;overflow:hidden">
        @foreach ($features as $i => $f)
          ...
          <div @click="off[{{ $i }}] = !off[{{ $i }}]" style="display:flex;gap:14px;padding:14px 16px;border-bottom:1px solid #16161B;cursor:pointer;align-items:flex-start">
            ...
            <div><div :style="{ color: off[{{ $i }}] ? '#6E6E79' : '#F2F2F5' }" style="font-weight:500">{{ $f }}</div><div style="font-size:13px;color:#8A8A94;margin-top:3px;line-height:1.45">{{ __('Included in the first version.') }}</div></div>
          </div>
        @endforeach
        <div style="padding:12px 16px;display:flex;gap:8px"><input name="extra" placeholder="{{ __('Anything else? e.g. “Customers should get a text when their order is ready”') }}" class="inp" style="flex:1;min-width:0;height:36px;border-radius:8px;font-size:13.5px"><button name="action" value="add" class="btn" style="height:36px">{{ __('Add') }}</button></div>
      </div>
      ...
      <div style="display:flex;justify-content:space-between;gap:10px">
        <a href="{{ route('studio.dashboard') }}" class="btn" style="height:40px;padding:0 16px;border-radius:9px">{{ __('← Back') }}</a>
        <button name="action" value="build" class="btn primary" :style="busy ? { opacity: .6, pointerEvents: 'none' } : {}" style="height:40px;padding:0 20px;border-radius:9px;font-size:14px">{{ __('Looks good — build it') }}</button>
      </div>
    </form>
    @endif
  </div>
